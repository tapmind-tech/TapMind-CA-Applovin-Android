...
object GeoProviderAppLovin {
    ...
    fun getPlacementName(
        customParams: Bundle?,
        defaultName: String
    ): PlacementInfoAppLovin {

        val placementName = customParams?.getString("placementName")
            ?.takeIf { it.isNotEmpty() }
            ?: customParams?.getString("placementname")
                ?.takeIf { it.isNotEmpty() }

        if (!placementName.isNullOrEmpty()) {
            return PlacementInfoAppLovin(
                placementName = placementName,
                placementFlag = 1
            )
        }

        val placementId = customParams?.getString("placement_id")
            ?.takeIf { it.isNotEmpty() }
        ...
        val label = customParams?.getString("label")
            ?.takeIf { it.isNotEmpty() }
            ?: customParams?.getString("lable")
                ?.takeIf { it.isNotEmpty() }
...
data class PlacementInfoAppLovin(
